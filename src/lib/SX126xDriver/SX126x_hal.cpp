...
void SX126xHal::reset(bool bootloader)
{
    DBGLN("SX126X Reset");
    ...
    if (GPIO_PIN_RST != UNDEF_PIN)
    {
        ...
        delay(300); // SX126X busy is high for 230ms after reset.  The WaitOnBusy timeout is only 1ms.  So this long delay is required.
        if (bootloader)
        {
            pinMode(GPIO_PIN_BUSY, INPUT);
            if (GPIO_PIN_RST_2 != UNDEF_PIN)
...
void ICACHE_RAM_ATTR SX126xHal::WriteCommand(uint16_t command, uint8_t *buffer, uint8_t size, SX12XX_Radio_Number_t radioNumber)
{
...
void ICACHE_RAM_ATTR SX126xHal::WriteCommand(uint16_t command, SX12XX_Radio_Number_t radioNumber)
{
...
void ICACHE_RAM_ATTR SX126xHal::ReadCommand(uint8_t *buffer, uint8_t size, SX12XX_Radio_Number_t radioNumber)
{
