...
#define RX_TIMEOUT_PERIOD_BASE_NANOS 1000000000 / 32768 // TODO check for SX126X
...
// This define refers to the High Frequency output on the SX1276.  But has been reused/repruposed for the SX126X.
// In ELRS V4 it should be changed to USE_RADIO_RFO_LP and refer to using the Low Power radio frequency output
// of both the SX1276 and SX126X.
...
bool SX126xDriver::Begin(uint32_t minimumFrequency, uint32_t maximumFrequency)
{
    ...
    uint8_t version[5] = {0};
    hal.WriteCommand(SX126X_SYSTEM_GET_VERSION_OC, SX12XX_Radio_1);
    hal.ReadCommand(version, sizeof(version), SX12XX_Radio_1);

    DBGLN("Read SX126X #1 Use Case (0x03 = SX126X): %d", version[2]);
    if (version[2] != 0x03)
    {
    DBGLN("SX126X #1 failed to be detected.");
        return false;
    }
    DBGLN("SX126X #1 Ready");

    if (GPIO_PIN_NSS_2 != UNDEF_PIN)
    {
        // Validate that the SX126X #2 is working.
        memset(version, 0, sizeof(version));
        hal.WriteCommand(SX126X_SYSTEM_GET_VERSION_OC, SX12XX_Radio_2);
        hal.ReadCommand(version, sizeof(version), SX12XX_Radio_2);

        DBGLN("Read SX126X #2 Use Case (0x03 = SX126X): %d", version[2]);
        if (version[2] != 0x03)
        {
        DBGLN("SX126X #2 failed to be detected.");
            return false;
        }
        DBGLN("SX126X #2 Ready");
    }

    //Clear Errors
    hal.WriteCommand(SX126X_SYSTEM_CLEAR_ERRORS_OC, SX12XX_Radio_All); // Remove later?  Might not be required???
...
*/
    ...
    uint8_t FBbuf[1] = {SX126X_RADIO_FALLBACK_FS};
    fallBackMode = SX126X_MODE_FS;
    hal.WriteCommand(SX126X_RADIO_SET_RX_TX_FALLBACK_MODE_OC, FBbuf, sizeof(FBbuf), SX12XX_Radio_All);

    // 7.2.12 SetRxBoosted
    uint8_t abuf[1] = {1};
    hal.WriteCommand(SX126X_RADIO_SET_RX_BOOSTED_OC, abuf, sizeof(abuf), SX12XX_Radio_All);
    ...
    if (OPT_USE_HARDWARE_DCDC)
    {
        // 5.1.1 SetRegMode
        uint8_t RegMode[1] = {1};    
        hal.WriteCommand(SX126X_SYSTEM_SET_REGMODE_OC, RegMode, sizeof(RegMode), SX12XX_Radio_All); // Enable DCDC converter instead of LDO
    }
#endif
    ...
    hal.WriteCommand(SX126X_SYSTEM_CALIBRATE_IMAGE_OC, CalImagebuf, sizeof(CalImagebuf), SX12XX_Radio_All);
...
void SX126xDriver::startCWTest(uint32_t freq, SX12XX_Radio_Number_t radioNumber)
{
    ...
    Config(SX126X_RADIO_LORA_BW_62, SX126X_RADIO_LORA_SF6, SX126X_RADIO_LORA_CR_4_8, freq, 12, false, 8, 0, false, 0, 0, radioNumber);
    CommitOutputPower();
    hal.WriteCommand(SX126X_RADIO_SET_TX_CW_OC, radioNumber);
}
                          ...
                          SX12XX_Radio_Number_t radioNumber)
{
    ...
    IQinverted = InvertIQ ? SX126X_RADIO_LORA_IQ_INVERTED : SX126X_RADIO_LORA_IQ_STANDARD;
    // IQinverted is always STANDARD for 900 and SX1276
    if (isSubGHz)
    {
        IQinverted = SX126X_RADIO_LORA_IQ_STANDARD;
    }

    SetRxTimeoutUs(interval);

    SetMode(SX126X_MODE_STDBY_RC, radioNumber);

    useFSK = setFSKModulation;
    
    // 8.1.1 SetPacketType
    uint8_t buf[1] = {useFSK ? SX126X_RADIO_PKT_TYPE_GFSK : SX126X_RADIO_PKT_TYPE_LORA};
    hal.WriteCommand(SX126X_RADIO_SET_PKT_TYPE_OC, buf, sizeof(buf), radioNumber);
    ...
    #if defined(DEBUG_FREQ_CORRECTION) // TODO Check if this available with the SX126X?
...
void SX126xDriver::ConfigModParamsFSK(uint32_t Bitrate, uint8_t BWF, uint32_t Fdev, SX12XX_Radio_Number_t radioNumber)
{
    ...
    buf[2] = Bitrate >> 8;
    buf[3] = Bitrate >> 0;
    buf[4] = SX126X_RADIO_GFSK_PULSE_SHAPE_OFF;             // Pulse Shape - 0x00: No filter applied
    buf[5] = BWF;
    buf[6] = Fdev >> 24;
    buf[7] = Fdev >> 16;
    buf[8] = Fdev >> 8;
    buf[9] = Fdev >> 0;
    hal.WriteCommand(SX126X_RADIO_SET_MODULATION_PARAM_OC, buf, sizeof(buf), radioNumber);    
}

void SX126xDriver::SetPacketParamsFSK(uint8_t PreambleLength, uint8_t PayloadLength, SX12XX_Radio_Number_t radioNumber)
{
    ...
    buf[2] = SX126X_RADIO_GFSK_PREAMBLE_DETECTOR_MIN_8BITS; // Pbl Detect - 0x04: Preamble detector length 8 bits
    buf[3] = 16;                                            // SyncWordLen defines the length of the Syncword in bits. By default, the Syncword is set to 0x9723522556536564
    buf[4] = SX126X_RADIO_GFSK_ADDRESS_FILTERING_DISABLE;   // Addr Comp - 0x00: Address Filtering Disabled
    buf[5] = SX126X_RADIO_GFSK_PKT_FIX_LEN;                 // PacketType - 0x00: Packet length is known on both sides
    buf[6] = PayloadLength;                                 // PayloadLen
    buf[7] = SX126X_RADIO_GFSK_CRC_OFF;                     // CrcType - 0x01: CRC_OFF (No CRC).
    buf[8] = SX126X_RADIO_GFSK_DC_FREE_WHITENING;           // DcFree - 0x01: SX127x/SX126x/LR11xx compatible whitening enable. 0x03: SX128x compatible whitening enable
    hal.WriteCommand(SX126X_RADIO_SET_PKT_PARAM_OC, buf, sizeof(buf), radioNumber);
}

void SX126xDriver::SetFSKSyncWord(uint8_t fskSyncWord1, uint8_t fskSyncWord2, SX12XX_Radio_Number_t radioNumber)
{
    ...
    hal.WriteCommand(SX126X_RADIO_SET_GFSK_SYNC_WORD_OC, synbuf, sizeof(synbuf), radioNumber);
}

void SX126xDriver::SetDioAsRfSwitch()
{
    // 4.2.1 SetDioAsRfSwitch
    uint8_t switchbuf[8];
    if (SX126X_RFSW_CTRL_COUNT == 8)
    {
        switchbuf[0] = SX126X_RFSW_CTRL[0]; // RfswEnable
        switchbuf[1] = SX126X_RFSW_CTRL[1]; // RfSwStbyCfg
        switchbuf[2] = SX126X_RFSW_CTRL[2]; // RfSwRxCfg
        switchbuf[3] = SX126X_RFSW_CTRL[3]; // RfSwTxCfg
        switchbuf[4] = SX126X_RFSW_CTRL[4]; // RfSwTxHPCfg
        switchbuf[5] = SX126X_RFSW_CTRL[5]; // RfSwTxHfCfg
        switchbuf[6] = SX126X_RFSW_CTRL[6]; // Unused
        switchbuf[7] = SX126X_RFSW_CTRL[7]; // RfSwWifiCfg - Each bit indicates the state of the relevant RFSW DIO when in Wi-Fi scanning mode or high frequency RX mode (LR1110_H1_UM_V1-7-1.pdf)
    }
    else
    {
    ...
    hal.WriteCommand(SX126X_SYSTEM_SET_DIO_AS_RF_SWITCH_OC, switchbuf, sizeof(switchbuf), SX12XX_Radio_All);
}
...
void SX126xDriver::CorrectRegisterForSF6(uint8_t sf, SX12XX_Radio_Number_t radioNumber)
{
    ...
    if ((SX126X_radio_lora_sf_t)sf == SX126X_RADIO_LORA_SF6)
    {
        uint8_t wrbuf[12];
        // Address
        wrbuf[0] = 0x00; // MSB
        ...
        wrbuf[10] = 0x00;
        wrbuf[11] = 0x00;
        hal.WriteCommand(SX126X_REGMEM_WRITE_REGMEM32_MASK_OC, wrbuf, sizeof(wrbuf), radioNumber);
    }
}
 ...
 ***/
void SX126xDriver::SetOutputPower(int8_t power, bool isSubGHz)
{
    uint8_t pwrNew;
    ...
    if (isSubGHz)
    {
        ...
        {
            pwrNew = constrain(power, SX126X_POWER_MIN_LP_PA, SX126X_POWER_MAX_LP_PA);
        }
        else
        {
            pwrNew = constrain(power, SX126X_POWER_MIN_HP_PA, SX126X_POWER_MAX_HP_PA);
        }
    ...
    {
        pwrNew = constrain(power, SX126X_POWER_MIN_HF_PA, SX126X_POWER_MAX_HF_PA);
...
void ICACHE_RAM_ATTR SX126xDriver::WriteOutputPower(uint8_t power, bool isSubGHz, SX12XX_Radio_Number_t radioNumber)
{
    uint8_t Pabuf[4] = {0};
    uint8_t Txbuf[2] = {power, SX126X_RADIO_RAMP_48_US};
    ...
    if (isSubGHz)
    {
        ...
        if (OPT_USE_SX1276_RFO_HF)
        {
            Pabuf[0] = SX126X_RADIO_PA_SEL_LP; // PaSel - 0x01: Selects the high power PA
            Pabuf[1] = SX126X_RADIO_PA_REG_SUPPLY_VREG; // RegPaSupply - 0x01: Powers the PA from VBAT. The user must use RegPaSupply = 0x01 whenever TxPower > 14
        ...
        {
            Pabuf[0] = SX126X_RADIO_PA_SEL_HP; // PaSel - 0x01: Selects the high power PA
            Pabuf[1] = SX126X_RADIO_PA_REG_SUPPLY_VBAT; // RegPaSupply - 0x01: Powers the PA from VBAT. The user must use RegPaSupply = 0x01 whenever TxPower > 14
    ...
    {
        Pabuf[0] = SX126X_RADIO_PA_SEL_HF; // PaSel - 0x02: Selects the high frequency PA
        Pabuf[1] = SX126X_RADIO_PA_REG_SUPPLY_VREG; // RegPaSupply - 0x01: Powers the PA from VBAT. The user must use RegPaSupply = 0x01 whenever TxPower > 14
    ...
    hal.WriteCommand(SX126X_RADIO_SET_PA_CFG_OC, Pabuf, sizeof(Pabuf), radioNumber);
    hal.WriteCommand(SX126X_RADIO_SET_TX_PARAMS_OC, Txbuf, sizeof(Txbuf), radioNumber);
}
...
void SX126xDriver::SetMode(SX126X_RadioOperatingModes_t OPmode, SX12XX_Radio_Number_t radioNumber)
{
    ...
    case SX126X_MODE_SLEEP:
        // 2.1.5.1 SetSleep
        hal.WriteCommand(SX126X_SYSTEM_SET_SLEEP_OC, buf, 5, radioNumber);
        break;

    case SX126X_MODE_STDBY_RC:
        //2.1.2.1 SetStandby
        buf[0] = 0x00;
        hal.WriteCommand(SX126X_SYSTEM_SET_STANDBY_OC, buf, 1, radioNumber);
        break;
    ...
    case SX126X_MODE_FS:
        // 2.1.9.1 SetFs
        hal.WriteCommand(SX126X_SYSTEM_SET_FS_OC, radioNumber);
        break;
    ...
    case SX126X_MODE_RX:
        ...
        buf[1] = timeout >> 8;
        buf[2] = timeout & 0xFF;
        hal.WriteCommand(SX126X_RADIO_SET_RX_OC, buf, 3, radioNumber);
        break;
    ...
    case SX126X_MODE_TX:
        ...
        hal.WriteCommand(SX126X_RADIO_SET_TX_OC, buf, 3, radioNumber);
        break;
...
void SX126xDriver::ConfigModParamsLoRa(uint8_t bw, uint8_t sf, uint8_t cr, SX12XX_Radio_Number_t radioNumber)
{
    // 8.3.1 SetModulationParams
    uint8_t buf[4];
    buf[0] = sf;
                                       ...
                                       uint8_t PayloadLength, uint8_t InvertIQ, SX12XX_Radio_Number_t radioNumber)
{
    ...
    buf[4] = SX126X_RADIO_LORA_CRC_OFF;
    buf[5] = InvertIQ;
    hal.WriteCommand(SX126X_RADIO_SET_PKT_PARAM_OC, buf, sizeof(buf), radioNumber);
}
...
void ICACHE_RAM_ATTR SX126xDriver::SetFrequencyHz(uint32_t freq, SX12XX_Radio_Number_t radioNumber)
{
    ...
    buf[2] = freq >> 8;
    buf[3] = freq & 0xFF;
    hal.WriteCommand(SX126X_RADIO_SET_RF_FREQUENCY_OC, buf, sizeof(buf), radioNumber);

    currFreq = freq;
}

void ICACHE_RAM_ATTR SX126xDriver::SetFrequencyReg(uint32_t freq, SX12XX_Radio_Number_t radioNumber)
{
    SetFrequencyHz(freq, radioNumber);
}

// 4.1.1 SetDioIrqParams
void SX126xDriver::SetDioIrqParams()
{
    uint8_t buf[8] = {0};
    buf[3] = SX126X_IRQ_TX_DONE | SX126X_IRQ_RX_DONE;
    hal.WriteCommand(SX126X_SYSTEM_SET_DIOIRQPARAMS_OC, buf, sizeof(buf), SX12XX_Radio_All);
}

// 3.4.1 GetStatus
uint32_t ICACHE_RAM_ATTR SX126xDriver::GetIrqStatus(SX12XX_Radio_Number_t radioNumber)
{
    uint8_t status[6] = {0};
    status[0] = SX126X_SYSTEM_CLEAR_IRQ_OC >> 8;
    status[1] = SX126X_SYSTEM_CLEAR_IRQ_OC & 0xFF;
    status[2] = 0xFF;
    status[3] = 0xFF;
    status[4] = 0xFF;
    status[5] = 0xFF;
...
void ICACHE_RAM_ATTR SX126xDriver::ClearIrqStatus(SX12XX_Radio_Number_t radioNumber)
{
    uint8_t buf[4];
    buf[0] = 0xFF;
    buf[1] = 0xFF;
    buf[2] = 0xFF;
    buf[3] = 0xFF;
    hal.WriteCommand(SX126X_SYSTEM_CLEAR_IRQ_OC, buf, sizeof(buf), radioNumber);
}
...
void ICACHE_RAM_ATTR SX126xDriver::TXnb(uint8_t * data, uint8_t size, SX12XX_Radio_Number_t radioNumber)
{
    ...
    if (useFEC)
    {
        ...
        // 3.7.4 WriteBuffer8
        hal.WriteCommand(SX126X_REGMEM_WRITE_BUFFER8_OC, FECBuffer, PayloadLength, radioNumber);
    }
    else
    {
        // 3.7.4 WriteBuffer8
        hal.WriteCommand(SX126X_REGMEM_WRITE_BUFFER8_OC, data, size, radioNumber);
    }

    SetMode(SX126X_MODE_TX, radioNumber);
...
bool ICACHE_RAM_ATTR SX126xDriver::RXnbISR(SX12XX_Radio_Number_t radioNumber)
{
    ...
    uint8_t buf[3] = {0};
    hal.WriteCommand(SX126X_RADIO_GET_RXBUFFER_STATUS_OC, radioNumber);
    ...
    hal.WriteCommand(SX126X_REGMEM_READ_BUFFER8_OC, inbuf, sizeof(inbuf), radioNumber);
...
{
    SetMode(SX126X_MODE_RX, SX12XX_Radio_All);
}
...
{
    uint8_t status[2] = {0};
    hal.WriteCommand(SX126X_RADIO_GET_RSSI_INST_OC, radioNumber);
    hal.ReadCommand(status, sizeof(status), radioNumber);
...
void ICACHE_RAM_ATTR SX126xDriver::GetLastPacketStats()
{
    ...
    if (GPIO_PIN_NSS_2 != UNDEF_PIN)
    {
        ...
        if(secondIrqStatus & SX126X_IRQ_RX_DONE)
        {
            // 7.2.11 GetRxBufferStatus
            uint8_t buf[3] = {0};
            hal.WriteCommand(SX126X_RADIO_GET_RXBUFFER_STATUS_OC, radio[secondRadioIdx]);
            ...
            hal.WriteCommand(SX126X_REGMEM_READ_BUFFER8_OC, inbuf, sizeof(inbuf), radio[secondRadioIdx]);
    ...
    hal.WriteCommand(SX126X_RADIO_GET_PKT_STATUS_OC, instance->processingPacketRadio | (gotRadio[secondRadioIdx] ? radio[secondRadioIdx] : 0));
