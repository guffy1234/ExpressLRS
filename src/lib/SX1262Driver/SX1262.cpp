...
bool SX1262Driver::Begin(uint32_t minimumFrequency, uint32_t maximumFrequency)
{
    // todo: handle uint32_t minimumFrequency, uint32_t maximumFrequency
	...
	hal.WriteCommand( SX1262_RADIO_SET_RFSWITCHMODE, 1); //dio2 as rf switch
	hal.WriteRegister( SX1262_REG_TX_CLAMP_CONFIG, hal.ReadRegister( SX1262_REG_TX_CLAMP_CONFIG ) | ( 0x1E ) );
	hal.WriteCommand( SX1262_RADIO_SET_STOPRXTIMERONPREAMBLE, false);
...
void SX1262Driver::startCWTest(uint32_t freq, SX12XX_Radio_Number_t radioNumber)
{
    Config(SX1262_LORA_BW_062, SX1262_LORA_SF6, SX1262_LORA_CR_4_8, freq, 12, false, 8, 0);
    CommitOutputPower();
    hal.WriteCommand(SX1262_RADIO_SET_TXCONTINUOUSWAVE, radioNumber);

    // todo: copied from 1276
//   SetPreambleLength(8);
                          ...
                          uint32_t flrcSyncWord, uint16_t flrcCrcSeed, uint8_t flrc)
{
	...
	currBW = bw;
    SetMode(SX1262_MODE_STDBY_RC);
    hal.WriteCommand(SX1262_RADIO_SET_PACKETTYPE, mode);
	...
	hal.WriteCommand( SX1262_RADIO_SET_LORASYMBTIMEOUT, 0);
}
...
void SX1262Driver::SetMode(SX1262_RadioOperatingModes_t OPmode, SX12XX_Radio_Number_t radioNumber)
{
    ...
    */
    ...
    case SX1262_MODE_SLEEP:
        hal.WriteCommand(SX1262_RADIO_SET_SLEEP, (uint8_t)0x01, radioNumber);
        break;

    case SX1262_MODE_STDBY_RC:
        hal.WriteCommand(SX1262_RADIO_SET_STANDBY, SX1262_STDBY_RC, radioNumber, 1500);
        break;

    // The DC-DC supply regulation is automatically powered in STDBY_XOSC mode.
    case SX1262_MODE_STDBY_XOSC:
        hal.WriteCommand(SX1262_RADIO_SET_STANDBY, SX1262_STDBY_XOSC, radioNumber, 50);
        break;

    case SX1262_MODE_FS:
        hal.WriteCommand(SX1262_RADIO_SET_FS, (uint8_t)0x00, radioNumber, 70);
        break;
