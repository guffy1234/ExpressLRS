...
public:
    ...
    void SetMode(SX1262_RadioOperatingModes_t OPmode, SX12XX_Radio_Number_t radioNumber = SX12XX_Radio_1); //ok
    void SetTxIdleMode() { 
        SetMode(SX1262_MODE_FS, SX12XX_Radio_All); 
	...
	void SetSyncWord(uint8_t syncWord); //ok
    void SetOutputPower(int8_t power); //ok
    void SetRxTimeoutUs(uint32_t interval); //ok
    ...
    void TXnb(uint8_t * data, uint8_t size, SX12XX_Radio_Number_t radioNumber = SX12XX_Radio_1); //ok
     /////////////Non-blocking RX related Functions///////////////
    void RXnb(SX1262_RadioOperatingModes_t rxMode = SX1262_MODE_RX); //ok
private:
